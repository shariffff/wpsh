...
Examples:
  ...
  wpsh domain add --server myserver --site mysite --domain www.example.com --ssl`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if input.IssueSSL {
			...
			if err != nil {
				...
				fmt.Println("You can issue SSL later with: wpsh domain ssl")
				os.Exit(1)
			}
			...
			fmt.Println()
			fmt.Println("To issue SSL later: wpsh domain ssl")
		}
	},
}
...
Examples:
  ...
  # Non-interactive mode (for automation/AI agents)
  wpsh domain remove --server myserver --site mysite --domain www.example.com --force`,
	Run: func(cmd *cobra.Command, args []string) {
		mgr, err := config.NewManager()
		if err != nil {
			outputError(cmd, "Failed to create config manager", err)
...
Examples:
  ...
  # Non-interactive mode (for automation/AI agents)
  wpsh domain ssl --server myserver --site mysite --domain www.example.com --email admin@example.com`,
	Run: func(cmd *cobra.Command, args []string) {
		mgr, err := config.NewManager()
		if err != nil {
			outputError(cmd, "Failed to create config manager", err)
